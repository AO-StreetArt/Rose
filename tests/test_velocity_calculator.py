...
def mock_memory_storage():
    """Create a mock memory storage for tests."""
    return Mock(spec=MemoryImageStorage)

@pytest.fixture
def mock_redis_storage():
    """Create a mock Redis storage for tests."""
    return Mock(spec=RedisImageStorage)

@pytest.fixture
def mock_image_comparator():
    """Create a mock image comparator for tests."""
    return Mock(spec=ImageComparator)

@pytest.fixture
def sample_detections():
    """Create sample detections for tests."""
    return [
        {
...
@pytest.fixture
def sample_frame():
    """Create a sample frame for tests."""
    return np.random.randint(0, 255, (480, 640, 3), dtype=np.uint8)

@pytest.fixture
def sample_depth_map():
    """Create a sample depth map for tests."""
    return np.random.rand(480, 640).astype(np.float32)

@pytest.fixture
def sample_segmentation_masks():
    """Create sample segmentation masks for tests."""
    return np.random.rand(2, 480, 640).astype(np.float32)

@pytest.yield_fixture
def velocity_calculator_default(mock_memory_storage, mock_image_comparator):
...
@pytest.fixture
def velocity_calculator_with_redis(mock_memory_storage, mock_redis_storage, mock_image_comparator):
...
class TestVelocityCalculator:
    ...
    def test_velocity_calculator_init_default(self, velocity_calculator_default):
        """Test VelocityCalculator initialization with default parameters."""       
        assert velocity_calculator_default.memory_storage is not None
        assert velocity_calculator_default.redis_storage is None
        assert velocity_calculator_default.image_comparator is not None
        assert velocity_calculator_default.frame_count == 0

    def test_velocity_calculator_init_with_redis(self, velocity_calculator_with_redis):
        """Test VelocityCalculator initialization with Redis storage."""
        assert velocity_calculator_with_redis.memory_storage is not None
        assert velocity_calculator_with_redis.redis_storage is not None
        assert velocity_calculator_with_redis.image_comparator is not None
        assert velocity_calculator_with_redis.frame_count == 0

    def test_velocity_calculator_init_with_custom_comparator(self, mock_memory_storage):
    ...
    def test_identify_objects_with_velocity_no_detections(self, velocity_calculator_default, sample_depth_map, sample_segmentation_masks, sample_frame):
        """Test velocity identification with no detections."""
        velocity_calculator_default.identify_objects_with_velocity(
            [], sample_depth_map, sample_segmentation_masks, sample_frame
        )
        
        # Should handle gracefully and return early
        # No assertions needed as method returns None

    def test_identify_objects_with_velocity_empty_memory_storage(self, velocity_calculator_default, mock_memory_storage, sample_frame):
        """Test velocity identification with empty memory storage."""
        # Mock empty memory storage
        mock_memory_storage.list_all_images.return_value = []
    ...
    def test_identify_objects_with_velocity_with_existing_data(self, velocity_calculator_default, mock_memory_storage, sample_frame):
        ...
        mock_memory_storage.list_all_images.return_value = mock_existing_objects
    ...
    def test_identify_objects_with_velocity_redis_sync(self, velocity_calculator_with_redis, mock_memory_storage, sample_frame):
    ...
    def test_identify_objects_with_velocity_redis_error(self, velocity_calculator_with_redis, mock_memory_storage, sample_frame):
    ...
    def test_calculate_object_depth_with_depth_map(self, velocity_calculator_default, sample_depth_map, sample_segmentation_masks):
        """Test object depth calculation with valid depth map."""
        bbox = [100, 100, 200, 200]
        depth = velocity_calculator_default._calculate_object_depth(
            bbox, sample_depth_map, sample_segmentation_masks
        )
        
        assert isinstance(depth, float)
        assert depth >= 0.0

    def test_calculate_object_depth_no_depth_map(self, velocity_calculator_default, sample_segmentation_masks):
        """Test object depth calculation with no depth map."""
        bbox = [100, 100, 200, 200]
        depth = velocity_calculator_default._calculate_object_depth(
            bbox, None, sample_segmentation_masks
        )
        
        assert depth == 0.0

    def test_calculate_object_depth_edge_coordinates(self, velocity_calculator_default, sample_depth_map, sample_segmentation_masks):
    ...
    def test_find_best_image_match_success(self, velocity_calculator_default, mock_memory_storage, mock_image_comparator, sample_frame):
        """Test finding best image match successfully."""
        # Mock stored image
        mock_stored_image = np.random.randint(0, 255, (100, 100, 3), dtype=np.uint8)
        mock_memory_storage.retrieve_image.return_value = mock_stored_image
        
        # Mock image comparison result
        mock_image_comparator.compare_images.return_value = {'similarity_score': 0.8}
        ...
        best_match = velocity_calculator_default._find_best_image_match(
            sample_frame[100:200, 100:200], matching_objects
        )
        
        assert best_match is not None
        assert best_match['key'] == 'test_key_1'

    def test_find_best_image_match_no_matches(self, velocity_calculator_default, mock_memory_storage, mock_image_comparator, sample_frame):
        ...
        mock_image_comparator.compare_images.return_value = {'similarity_score': 0.3}
        ...
        best_match = velocity_calculator_default._find_best_image_match(
            sample_frame[100:200, 200:300], matching_objects
        )
        
        assert best_match is None

    def test_find_best_image_match_empty_matching_objects(self, velocity_calculator_default, sample_frame):
        """Test finding best image match with empty matching objects."""
        best_match = velocity_calculator_default._find_best_image_match(
            sample_frame[100:200, 100:200], []
        )
    ...
    def test_update_metadata_with_match_id_success(self, velocity_calculator_default, mock_memory_storage):
        """Test updating metadata with match ID successfully."""
        # Mock successful metadata update
        mock_memory_storage.update_metadata.return_value = True
        
        success = velocity_calculator_default._update_metadata_with_match_id('test_key', 'match_123')
        
        assert success is True
        mock_memory_storage.update_metadata.assert_called_once_with('test_key', {'match_id': 'match_123'})

    def test_update_metadata_with_match_id_failure(self, velocity_calculator_default, mock_memory_storage):
        """Test updating metadata with match ID failure."""
        # Mock failed metadata update
        mock_memory_storage.update_metadata.return_value = False
        
        success = velocity_calculator_default._update_metadata_with_match_id('test_key', 'match_123')
        
        assert success is False

    def test_store_new_detection_success(self, velocity_calculator_default, mock_memory_storage, sample_frame):
        ...
        object_region = sample_frame[100:200, 100:200]
        depth = 2.0
        
        velocity_calculator_default._store_new_detection(object_region, detection, depth)
        
        # Should store image in memory storage
        mock_memory_storage.store_image.assert_called_once()

    def test_store_new_detection_invalid_bbox(self, velocity_calculator_default, mock_memory_storage, sample_frame):
        ...
        mock_memory_storage.store_image.assert_not_called()

    def test_get_velocity_stats_no_objects(self, velocity_calculator_default, mock_memory_storage):
    ...
    def test_get_velocity_stats_with_objects(self, velocity_calculator_default, mock_memory_storage):
        ...
        mock_memory_storage.list_all_images.return_value = mock_velocity_objects
    ...
    def test_get_velocity_stats_mixed_metadata(self, velocity_calculator_default, mock_memory_storage):
    ...
    def test_get_velocity_stats_exception_handling(self, velocity_calculator_default, mock_memory_storage):
        """Test getting velocity stats with exception handling."""
        # Mock exception in search
        mock_memory_storage.list_all_images.side_effect = Exception("Storage error")
        
        stats = velocity_calculator_default.get_velocity_stats()
        
        assert stats == {}

    def test_process_detection_for_velocity_success(self, velocity_calculator_default, mock_memory_storage, mock_image_comparator, sample_depth_map, sample_segmentation_masks, sample_frame, sample_detections):
        """Test processing detection for velocity tracking successfully."""
        # Mock search results
        mock_matching_objects = [
            {'key': 'test_key_1', 'metadata': {'class_name': 'person'}}
        ]
        mock_memory_storage.search_by_tags.return_value = mock_matching_objects
        ...
        detection = sample_detections[0]  # Use first detection
        
        velocity_calculator_default._process_detection_for_velocity(
            detection, sample_depth_map, sample_segmentation_masks, sample_frame
        )
        
        # Should process detection and store results
        mock_memory_storage.store_image.assert_called()

    def test_process_detection_for_velocity_invalid_bbox(self, velocity_calculator_default, mock_memory_storage, sample_depth_map, sample_segmentation_masks, sample_frame):
        ...
        mock_memory_storage.search_by_tags.assert_not_called()

    def test_process_detection_for_velocity_no_matches(self, velocity_calculator_default, mock_memory_storage, sample_depth_map, sample_segmentation_masks, sample_frame, sample_detections):
        """Test processing detection for velocity with no matching objects."""
        # Mock empty search results
        mock_memory_storage.search_by_tags.return_value = []
    ...
    def test_populate_initial_detections_success(self, velocity_calculator_default, mock_memory_storage, sample_depth_map, sample_segmentation_masks, sample_frame, sample_detections):
        """Test populating initial detections successfully."""
        velocity_calculator_default._populate_initial_detections(
            sample_detections, sample_depth_map, sample_segmentation_masks, sample_frame
        )
        
        # Should store initial detections
        mock_memory_storage.store_image.assert_called()

    def test_populate_initial_detections_invalid_bbox(self, velocity_calculator_default, mock_memory_storage, sample_depth_map, sample_segmentation_masks, sample_frame):
        ...
        velocity_calculator_default._populate_initial_detections(
            invalid_detections, sample_depth_map, sample_segmentation_masks, sample_frame
        )

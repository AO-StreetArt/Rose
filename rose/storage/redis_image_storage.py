...
class RedisImageStorage:
    ...
    def __init__(
        ...
        """
        # Use separate Redis clients for binary data (images) and text data
        # (metadata, tags, etc.)
    ...
    def store_image(
        ...
        try:
            ...
            self.text_client.set(
                f"{self.METADATA_PREFIX}{key}", json.dumps(metadata, default=str)
            )
            ...
            if ttl > 0:
                ...
                self.text_client.expire(f"{self.METADATA_PREFIX}{key}", ttl * 3600)
            ...
            if tags:
                for tag in tags:
                    ...
                    if ttl > 0:
                        self.text_client.expire(tag_key, ttl * 3600)

            # Add to global index
            self.text_client.sadd(f"{self.INDEX_PREFIX}all", key)
    ...
    def search_by_tags(
        ...
        try:
            if operator == "OR":
                ...
                for tag in tags:
                    tag_key = f"{self.TAG_PREFIX}{tag}"
                    keys = self.text_client.smembers(tag_key)
    ...
    def list_all_images(self, limit: int = 100) -> List[Dict[str, Any]]:
        ...
        try:
            all_keys = self.text_client.smembers(f"{self.INDEX_PREFIX}all")
            results = []
    ...
    def delete_image(self, key: str) -> bool:
        ...
        try:
            ...
            self.text_client.srem(f"{self.INDEX_PREFIX}all", key)

            # Delete image and metadata
            self.redis_client.delete(f"{self.IMAGE_PREFIX}{key}")
            self.text_client.delete(f"{self.METADATA_PREFIX}{key}")
